...
<div class="container my-1">
    ...
    <form action="{{URL::to('/diaries/'.$diary->id)}}" method="post">
        @csrf
        <div class="form-group">
          <label for="title">Título</label>
          <input type="text" class="form-control" name="title" id="title" placeholder="Ingresá aca el titulo" value="{{$diary->title}}">
        </div>
        <div class="form-group">
          <label for="body">Entrada de diario</label>
          <textarea class="form-control" name="body" id="body" rows="3" placeholder="Como fue tu dia?">{{$diary->body}}</textarea>
        </div>
